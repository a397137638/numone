...
(function ($) {
    $.extend({
        ...
        operate: {
			...
			// 绘制流程图
			creat: function() {
				var url = "/create?name=activiti&key=123456";
				window.open(url);
			},
			// 修改流程图
			editor: function(id) {
				var url = "/editor?modelId="+id;
				window.open(url);
			},
			// 发布
			pub: function(id) {
				$.modal.confirm("确定发布" + $.table._option.modalName + "信息吗？", function() {
					var url = "/publish";
					var data = { "modelId": id };
					$.operate.submit(url, "post", "json", data);
				});
			...
			// 撤消
			revokePublish: function(id) {
				$.modal.confirm("确定撤消" + $.table._option.modalName + "信息吗？", function() {
					var url = "/revokePublish";
					var data = { "modelId": id };
					$.operate.submit(url, "post", "json", data);
				});
			},
			// 查看流程图
			exportPng: function(id) {
				var url = "/exportPng?modelId="+id;
				window.open(url);
			},
			// 删除模型
			delete: function(id) {
				$.modal.confirm("确定删除" + $.table._option.modalName + "信息吗？", function() {
					var url = "/delete";
					var data = { "modelId": id };
					$.operate.submit(url, "post", "json", data);
				});
